...
getSegments (Orange _ x) = x
getSegments _ = 0
bloodOrangeSegments fruits = sum [getSegments fruit | fruit <- fruits]

-- 3.
worms :: [Fruit] -> Int
getWorms :: Fruit -> Int
getWorms (Apple _ True) = 1
getWorms _ = 0
worms fruits = sum [getWorms fruit | fruit <- fruits]
...
prop_taut1 p = (((tautology p) || (satisfiable (Not p))) == True)
...
p4 = ((p :->: q) :&: (p :<->: q))
p5 = ((p :->: q) :&: (p :&: (Not q)))
p6 = ((p :<->: q) :&: ((p :&: (Not q)) :|: ((Not p) :&: q))) 


-- 7.
equivalent :: Prop -> Prop -> Bool
equivalent p q = and [eval e p == eval e q | e <- envs (names (p :&: q))]
...
subformulas (Not x) = [Not x] ++ subformulas x
subformulas expr@(x :&: y) = [expr] ++ subformulas x ++ subformulas y
subformulas expr@(x :|: y) = [expr] ++ subformulas x ++ subformulas y
subformulas expr@(x :->: y) = [expr] ++ subformulas x ++ subformulas y
subformulas expr@(x :<->: y) = [expr] ++ subformulas x ++ subformulas y
